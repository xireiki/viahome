...
(function(root, factory) {
    ...
    for (let i in this.device.configurations) {
      ...
      for (let j in conf.interfaces) {
        ...
        for (let k in intf.alternates) {
          ...
          if (filter.classCode == alt.interfaceClass && filter.subclassCode == alt.interfaceSubclass && filter.protocolCode == alt.interfaceProtocol) {
            return {
              conf: conf,
              intf: intf,
              alt: alt
  ...
  Adb.WebUSB.Transport.prototype.connectAdb = function(banner, auth_user_notify = null) {
    ...
    return this.getDevice({
      ...
      return keys.then(keys => do_auth(adb, keys, key_idx++, response.data.buffer, do_auth_response, auth_user_notify));
    }))
      .then(response => {
      if (response.cmd != "CNXN") throw new Error("Failed to connect with '" + banner + "'");
      console.log('version', response.arg0);
      if (response.arg0 != VERSION && response.arg0 != VERSION_NO_CHECKSUM) throw new Error("Version mismatch: " + response.arg0 + " (expected: " + VERSION + " or " + VERSION_NO_CHECKSUM + ")");
      ...
      adb.banner = new TextDecoder("utf-8")
  ...
  Adb.WebUSB.Device.prototype.send = function(data) {
    ...
    if (data != null && data.length > this.max_payload) throw new Error("data is too big: " + data.length + " bytes (max: " + this.max_payload + " bytes)");
  ...
  Fastboot.WebUSB.Device.prototype.send = function(data) {
    ...
    if (data != null && data.length > this.max_datasize) throw new Error("data is too big: " + data.length + " bytes (max: " + this.max_datasize + " bytes)");
  ...
  Adb.Message.send = function(device, message) {
    ...
    if (message.data != null) {
      ...
      if (len > device.max_payload) throw new Error("data is too big: " + len + " bytes (max: " + device.max_payload + " bytes)");
    }
      ...
      if (response.cmd == "WRTE") {
        ...
        if (cmd == "OKAY" || cmd == "DATA" || cmd == "DONE" || cmd == "FAIL") {
          ...
          return stream.send("OKAY")
            ...
            if (response.data.byteLength < len) throw new Error("expected at least " + len + ", got " + response.data.byteLength);
  ...
  Adb.Stream.open = function(device, service) {
    ...
    return m.send_receive(device)
      ...
      if (response.arg1 != local_id) return Adb.Message.receive(device)
        .then(do_response);
  ...
  Adb.Stream.prototype.receive = function() {
    return Adb.Message.receive(this.device)
      ...
      if (response.arg0 != 0 && response.arg0 != this.remote_id) throw new Error("Incorrect arg0: 0x" + toHex32(response.arg0) + " (expected 0x" + toHex32(this.remote_id) + ")");
      if (this.local_id != 0 && response.arg1 != this.local_id) throw new Error("Incorrect arg1: 0x" + toHex32(response.arg1) + " (expected 0x" + toHex32(this.local_id) + ")");
      return response;
    });
  };
  ...
  Adb.Stream.prototype.pull = function(filename) {
    ...
    return frame.send_receive(this)
      ...
      if (response.data.byteLength != len) throw new Error("PULL DATA failed on " + filename + ": " + response.data.byteLength + "!=" + len);
  ...
  Adb.Stream.prototype.push = function(file, filename, mode, on_progress = null) {
    ...
    return read_blob(file)
      .then(data => this.push_start(filename, mode)
      .then(() => {
  ...
  function get_ep_num(endpoints, dir, type = "bulk") {
    let e, ep;
    for (e in endpoints)
    if (ep = endpoints[e], ep.direction == dir && ep.type == type) return ep.endpointNumber;
  ...
  function do_auth(adb, keys, key_idx, token, do_auth_response, auth_user_notify) {
    ...
    if (key_idx < keys.length) {
      ...
      return seq.then(() => crypto.subtle.sign({
        name: "RSASSA-PKCS1-v1_5"
  ...
  function init_db() {
    ...
    req.onupgradeneeded = function(event) {
      ...
      if (Adb.Opt.debug) console.log("DB: migrating from version " + event.oldVersion + " to " + event.newVersion + "...");
  ...
  function load_keys(db) {
    ...
    return promisify(transaction, "oncomplete")
